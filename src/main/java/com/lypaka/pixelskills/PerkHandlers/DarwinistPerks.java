...
import com.lypaka.pixelskills.Skills.Darwinist;
...
public class DarwinistPerks {
    ...
    private static void givePerk (String task, Player player) throws ObjectMappingException {
        ...
        String[] modifier = PerkGetters.getPerkModifiers(8, "").get("Modifier").split(" ");
        String function = modifier[0];
        double num;

        if (modifier[1].equalsIgnoreCase("%player-level%")) {

            num = AccountsHandler.getLevel("Darwinist", player);
        ...
        String[] stats = PerkGetters.getPerkModifiers(8, "").get("Modify-Stats").split(", ");

        for (String stat : stats) {

            IVModifier.modifyIVs(Darwinist.pokemon, stat, num, function);

        }

        player.sendMessage(FancyText.getFancyText(MessageHandlers.getPerkMessage(GeneralGetters.getConfigFromSkill("Darwinist"), "")));

    }

    private static void giveCustomPerk (Player player) throws ObjectMappingException {

        int options = PerkGetters.getCustomPerkAmount(8);
        ...
        if (options > 1) {
            ...
            map = PerkGetters.getCustomPerkMap(8, rng);
            perkNum = rng;

        } else {

            map = PerkGetters.getCustomPerkMap(8, 1);
            perkNum = 1;
        ...
        if (modifier[1].equalsIgnoreCase("%player-level%")) {
            ...
            modNum = AccountsHandler.getLevel("Darwinist", player);
            ...
            case "item":
                if (prize.contains(", ")) {
                    ...
                    for (String prz : prizes) {
                        ...
                        player.sendMessage(FancyText.getFancyText(MessageHandlers.getCustomPerkMessage(8, perkNum)
