...
import com.lypaka.pixelskills.Skills.Botanist;
...
public class BotanistPerks {
    ...
    private static void givePerk (String task, Player player) throws ObjectMappingException {
        ...
        switch (task) {
            ...
            case "Planting-Berries":
                ...
                int amount = Integer.parseInt(PerkGetters.getPerkModifiers(3, task).get("Amount"));
                String[] modifier = PerkGetters.getPerkModifiers(3, task).get("Modifier").split(" ");
                ...
                if (modifier[1].equalsIgnoreCase("%player-level%")) {
                    ...
                    num = AccountsHandler.getLevel("Botanist", player);
                ...
                String item = Botanist.getItem();
                Sponge.getCommandManager().process(Sponge.getServer().getConsole(), "give " + player.getName() + " " + item + " " + (int) result);
                player.sendMessage(FancyText.getFancyText(MessageHandlers.getPerkMessage(GeneralGetters.getConfigFromSkill("Botanist"), task)
    ...
    private static void giveCustomPerk (Player player) throws ObjectMappingException {
        ...
        int options = PerkGetters.getCustomPerkAmount(3);
        ...
        if (options > 1) {
            ...
            map = PerkGetters.getCustomPerkMap(3, rng);
            perkNum = rng;

        } else {

            map = PerkGetters.getCustomPerkMap(3, 1);
            perkNum = 1;
        ...
        if (modifier[1].equalsIgnoreCase("%player-level%")) {
            ...
            modNum = AccountsHandler.getLevel("Botanist", player);
            ...
            case "item":
                if (prize.contains(", ")) {
                    ...
                    for (String prz : prizes) {
                        ...
                        player.sendMessage(FancyText.getFancyText(MessageHandlers.getCustomPerkMessage(3, perkNum)
                                .replace("%number%", String.valueOf(result))
                                .replace("%prize%", prz)
                        ));
