...
import com.lypaka.pixelskills.Skills.Collector;
...
public class CollectorPerks {
    ...
    private static void givePerk (String task, Player player) throws ObjectMappingException {
        ...
        String[] modifier = PerkGetters.getPerkModifiers(6, task).get("Modifier").split(" ");
        ...
        if (modifier[1].equalsIgnoreCase("%player-level%")) {
            ...
            num = AccountsHandler.getLevel("Collector", player);
        ...
        for (String stat : stats) {
            ...
            IVModifier.modifyIVs(Collector.getPokeToModify(), stat, num, function);

        }

        player.sendMessage(FancyText.getFancyText(MessageHandlers.getPerkMessage(GeneralGetters.getConfigFromSkill("Collector"), task)));

    }

    private static void giveCustomPerk (Player player) throws ObjectMappingException {

        int options = PerkGetters.getCustomPerkAmount(6);
        ...
        if (options > 1) {
            ...
            map = PerkGetters.getCustomPerkMap(6, rng);
            perkNum = rng;

        } else {

            map = PerkGetters.getCustomPerkMap(6, 1);
            perkNum = 1;
        ...
        if (modifier[1].equalsIgnoreCase("%player-level%")) {
            ...
            modNum = AccountsHandler.getLevel("Artificer", player);
            ...
            case "item":
                if (prize.contains(", ")) {
                    ...
                    for (String prz : prizes) {
                        ...
                        player.sendMessage(FancyText.getFancyText(MessageHandlers.getCustomPerkMessage(6, perkNum)
