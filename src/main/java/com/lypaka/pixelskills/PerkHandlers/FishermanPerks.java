...
import com.lypaka.pixelskills.Skills.Fisherman;
...
public class FishermanPerks {
    ...
    private static void givePerk (String task, Player player) throws ObjectMappingException {
        ...
        String[] modifier = PerkGetters.getPerkModifiers(9, "").get("Modifier").split(" ");
        ...
        if (modifier[1].equalsIgnoreCase("%player-level%")) {
            ...
            num = AccountsHandler.getLevel("Fisherman", player);
        ...
        String[] stats = PerkGetters.getPerkModifiers(9, "").get("Modify-Stats").split(", ");

        for (String stat : stats) {

            IVModifier.modifyIVs(Fisherman.pokemon, stat, num, function);

        }

        player.sendMessage(FancyText.getFancyText(MessageHandlers.getPerkMessage(GeneralGetters.getConfigFromSkill("Fisherman"), "")));

    }

    private static void giveCustomPerk (Player player) throws ObjectMappingException {

        int options = PerkGetters.getCustomPerkAmount(9);
        ...
        if (options > 1) {
            ...
            map = PerkGetters.getCustomPerkMap(9, rng);
            perkNum = rng;

        } else {

            map = PerkGetters.getCustomPerkMap(9, 1);
            perkNum = 1;
        ...
        if (modifier[1].equalsIgnoreCase("%player-level%")) {
            ...
            modNum = AccountsHandler.getLevel("Fisherman", player);
            ...
            case "item":
                if (prize.contains(", ")) {
                    ...
                    for (String prz : prizes) {
                        ...
                        player.sendMessage(FancyText.getFancyText(MessageHandlers.getCustomPerkMessage(9, perkNum)
